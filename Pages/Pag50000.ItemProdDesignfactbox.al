page 50000 "ABP Item Prod. Design factbox"
{
    ApplicationArea = All;
    ...
    layout
    {
        area(Content)
        {
            ...
            field(RoutinghVersion; GetRoutingVerison(Rec))
            {
                ...
                begin
                    if RoutingVersion.Get(GetRoutingNo(Rec), GetRoutingVerison(Rec)) then
            ...
            field(ProdBomVerison; GetProdBomVerison(Rec))
            {
                ...
                begin
                    if ProductionBOMVersion.Get(GetProdBomNo(Rec), GetProdBomVerison(Rec)) then
                        Page.RunModal(page::"Production BOM Version", ProductionBOMVersion);
                end;
            }
        }
    }
    procedure GetRoutingNo(Item: Record Item): Code[20]

    begin
        exit(Item."Routing No.");
    end;

    procedure GetProdBomNo(Item: Record Item): Code[20]
    begin
    ...
    local procedure GetRoutingVerison(Item: Record Item): code[20]
    begin
        exit(VersionMgt.GetRtngVersion(Item."Routing No.", WORKDATE, TRUE));
    end;

    local procedure GetProdBomVerison(Item: Record Item): code[20]
    begin
